...
type
  NifKind* = enum
    ...
    LineInfoLit       # reserved for future line-info adapters
...
proc closeTag*(b: var TokenBuf) =
  ...
  let count = uint64(b.len - p - 1)
  if count <= uint64(InlineJumpCap):
    b.data[p].setJump uint32(count)
  else:
    # Subtree too big for a single 19-bit jump field. Splice an
    # ExtendedSuffix immediately after the TagLit (shifts the body right
    # by 1). The TagLit keeps the low 19 bits of the new count; the
    # suffix carries the high bits. Any still-open outer ancestors in
    # `openTags` have positions < p, so their stored positions stay
    # valid — they'll see the extra token in their own count when they
    # close.
    let newCount = count + 1   # the inserted suffix itself counts
    let highBits = uint32(newCount shr JumpBits)
    let lowBits  = uint32(newCount and uint64(InlineJumpCap))
    assert highBits <= PayloadMask,
           "subtree exceeds 47-bit jump (size " & $newCount & ")"
    ensureCap(b)
...
proc addAcrossPools(dest: var TokenBuf; c: var Cursor) =
  ...
  ## into `dest`, re-interning literals (via the literals pool) and tag
  ## names (via the tag pool). Recurses through children.
  let srcPool = c.pool
  let srcTags = c.tags
  case c.kind
  of TagLit:
    ...
    dest.buildTree TagId(newTag):
      c.into:
        while c.hasMore:
          addAcrossPools(dest, c)
  of StrLit:    dest.addStrLit  strVal(c, srcPool);          c.inc
  of IntLit:    dest.addIntLit  intVal(c);                   c.inc
  of UIntLit:   dest.addUIntLit uintVal(c);                  c.inc
  of FloatLit:  dest.addFloatLit floatVal(c);                c.inc
  of Symbol:    dest.addSymUse  symName(c, srcPool);         c.inc
  of SymbolDef: dest.addSymDef  symName(c, srcPool);         c.inc
  of Ident:     dest.addIdent   strVal(c, srcPool);          c.inc
  of CharLit:   dest.addCharLit charLit(c);                  c.inc
  of DotToken, LineInfoLit:
    dest.add c.load; c.inc
  of ExtendedSuffix:
...
when isMainModule:
  ...
  block extended_suffix_jump:
    ...
    test c.cursorJump, uint64(big) + 1'u64       # body + the suffix slot
    c.skip
    test c.rem, 0
